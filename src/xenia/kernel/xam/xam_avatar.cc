...
bool LoadAvatarAssetPack(KernelState* kernel_state) {
  ...
  if (!is_loaded || !legacy_is_loaded) {
    ...
    X_RESULT result =
        content_manager->OpenContent("AvatarAssetPack", content_data);
    if (XSUCCEEDED(result)) {
      if (!is_loaded) {
...
dword_result_t XamAvatarGetMetadataLocalUser_entry(
    dword_t user_index, pointer_t<X_AVATAR_METADATA> metadata,
    pointer_t<XAM_OVERLAPPED> overlapped) {
  auto run = [metadata]() -> X_RESULT {
    //std::memcpy(metadata, gibbed_avatar, countof(gibbed_avatar));
    return X_ERROR_FUNCTION_FAILED;
  };
  if (!overlapped) {
    return run();
...
dword_result_t XamAvatarGetMetadataRandom_entry(
  ...
  auto run = [metadata_count, metadatas]() -> X_RESULT {
    /*for (uint32_t i = 0; i < metadata_count; ++i) {
      metadatas[i] = {};
      metadatas[i].magic = 'XENI';
      std::memcpy(metadatas[i].guid, kGenericAvatarGuid,
                  sizeof(metadatas[i].guid));
    }
    return X_ERROR_SUCCESS;*/
    return X_ERROR_FUNCTION_FAILED;
  };
...
dword_result_t XamAvatarSetMetadata_entry(dword_t a1, dword_t a2, dword_t a3) {
  return X_ERROR_SUCCESS;
}
